...
spectrum(initial_data, output_data, configurations::NonVacuumOTEConfigurations; kwargs...) = spectrum(initial_data, output_data, configurations, configurations.camera; kwargs...)
...
function spectrum(initial_data, 
    ...
    Fobs, _ = observed_specific_intensities(initial_data, output_data, configurations, energies)
    ...
    @sync map(chunks) do chunk
        Threads.@spawn begin
            ...
            for i in chunk
                ...
                observer_rest_frame_energy = scalar_product(ki, cache.observer_four_velocity, cache.observer_metric)
                nu = scalar_product(ki, cache.observer_four_velocity, cache.observer_metric)
                nn = scalar_product(ki, cache.flux_direction, cache.observer_metric)
                @. Fobs[:, i] = (observation_energies*observer_rest_frame_energy)^3*output_data[9+NE:end,i]
                Fobs[:,i] *= nu*nn*dΩ[i]        
            end
        end
    end
