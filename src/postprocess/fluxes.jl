...
function fluxes!(F::AbstractArray, configurations, camera::PinholeCamera, initial_data::AbstractMatrix, output_data::AbstractMatrix; observer_four_velocity=nothing, flux_direction=nothing, tasks_per_thread::Int=2)
    fluxes_checks(F, initial_data, output_data, configurations)
    dΩ = pixel_solid_angles(camera)
    itr = axes(F, ndims(F))
    tmap(fluxes_task, itr, F, initial_data, output_data, configurations, dΩ; tasks_per_thread=tasks_per_thread, observer_four_velocity=observer_four_velocity, flux_direction=flux_direction)
    return nothing
end

fluxes_task(chunk, F::AbstractVector, initial_data, output_data, configurations, dΩ; kwargs...) = begin
...
function fluxes_checks(F::AbstractArray, initial_data::AbstractMatrix, output_data::AbstractMatrix, configurations::VacuumOTEConfigurations)
    ...
    size(initial_data, 2) == nrays || throw(DimensionMismatch("The number of rays in the initial and output data must be the same as the length of F."))
    number_of_pixels(configurations.camera) == nrays || throw(DimensionMismatch("The number of pixels in the camera must be the same as the length of F."))
    return nothing 
end
