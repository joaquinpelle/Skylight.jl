...
function surface_adapted_triad!(triad,
    time_vector,
    metric,
    metric_inverse,
    position,
...
function orthonormalize!(dyad, time_vector, space_vector, metric)
    ...
    e1 .+= time_vector * scalar_product(e1, time_vector, metric) -
           space_vector * scalar_product(e1, space_vector, metric)
    normalize_spacelike!(e1, metric)

    e2 .+= time_vector * scalar_product(e2, time_vector, metric) -
           space_vector * scalar_product(e2, space_vector, metric) -
           e1 * scalar_product(e2, e1, metric)
...
function orthonormalize!(triad, time_vector, metric)
    ...
    e3 .+= time_vector * scalar_product(e3, time_vector, metric) -
           e1 * scalar_product(e3, e1, metric) - e2 * scalar_product(e3, e2, metric)
...
function coordinate_components_from_tetrad_components!(vectors, tetrad)
    ...
    size(vectors, 1) == 4 ||
        throw(DimensionMismatch("First dimension of vectors must be 4"))
