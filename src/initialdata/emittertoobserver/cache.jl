...
function metric_and_tetrad!(cache::ETOInitialDataCache,
    position,
    configurations::AbstractConfigurations)
    metric_and_tetrad!(cache,
        position,
        configurations,
        opaque_interior_surface_trait(configurations.radiative_model))
    return nothing
end
...
function metric_and_tetrad!(cache::ETOInitialDataCache,
    ...
    rest_frame_four_velocity!(time_vector,
        position,
        metric,
        spacetime,
        model,
