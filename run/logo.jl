...
function main(original_image, filename; spin=0.0, d_obs_screen, rel_bh_pos, bh_size, xaperture, n_factor=1)
    ...
    model = VerticalScreen(x=xscreen, horizontal_side = ximage, vertical_side = (ny/nx)*ximage) 
    ...
    for j in 1:Ny
        for i in 1:Nx
            ...
            if !(x≈model.x) || abs(y) > 0.5*model.horizontal_side || abs(z) > 0.5*model.vertical_side
                new_image[i,j] = zero(eltype(original_image))
                continue
            end

            in = floor(Int, 1+nx*(y+0.5*model.horizontal_side)/model.horizontal_side)
...
main(original_image, "new_image.png"; d_obs_screen = 230.0, rel_bh_pos = 3/23, bh_size = 1/15.0, xaperture = 10, n_factor=3)